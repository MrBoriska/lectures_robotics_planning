...
<div class="card card-accent">
  <h3>🎯 От непрерывного мира к графам</h3>
  <p class="text-sm">
    Физический мир робота непрерывен, однако детерминированные вычислительные алгоритмы требуют <strong>дискретных математических структур</strong>.
  </p>
  <p class="text-sm">
...
<div class="card">
  ...
  <ul class="text-sm">
    <li><strong>Как связать точки среды?</strong> Точные топологические карты: граф видимости (кратчайший) vs диаграмма Вороного (безопаснейший).</li>
    <li><strong>Как ускорить поиск?</strong> Дейкстра vs $A^*$: условия допустимости и монотонности эвристик.</li>
    <li><strong>Как устранить сеточные изломы?</strong> Сглаженный поиск Any-Angle ($\text{Theta}^*$).</li>
    <li><strong>Как учесть время и других роботов?</strong> Space-Time $A^*$, таблица резервирования и масштабируемый алгоритм CBS.</li>
    <li><strong>Где место нейросетей?</strong> Обучение эвристик без потери гарантий оптимальности.</li>
  </ul>
</div>
...
<div class="card card-success mt-2">
  ...
  <p class="text-sm" style="margin-bottom: 0;">
    Строить и оптимизировать графовые представления пространства, применять алгоритмы $\text{A}^*$ и $\text{Theta}^*$ без искусственных искажений метрики, а также координировать движение группы роботов во времени с разрешением пространственно-временных коллизий.
  </p>
</div>
