...
## 5. Алгоритм Дейкстры: постановка <span class="badge badge-time">22–26 мин</span>
...
Для каждого узла — два числа:

- $d(v)$ — **длина лучшего известного пути** от старта до $v$. В начале $d(\text{старт}) = 0$, у остальных $d = \infty$.
- $\text{родитель}(v)$ — из какого узла в $v$ пришли по этому лучшему пути.

Путь в конце читается с конца: от цели по родителям до старта.

</div>

<div class="card card-accent">

### Содержание одной итерации

1. **Извлечь** из очереди узел $u$ с наименьшим $d$. Его значение уже окончательное.
2. **Ослабить** (relax) каждое ребро $(u, v)$ длины $w$: если $d(u) + w < d(v)$, то мы нашли путь короче — записываем $d(v) \leftarrow d(u) + w$ и $\text{родитель}(v) \leftarrow u$.

Всё. Повторять, пока не извлечём цель.
...
**Обоснование окончательности.** Все веса рёбер неотрицательны, поэтому любой другой маршрут до $u$ шёл бы через узел с бо́льшим $d$ — то есть был бы длиннее. Это ломается при отрицательных весах: там нужен Беллман–Форд.
