...
<span class="badge badge-green">Кинодинамика и предиктивное управление</span>
...
В предыдущих разделах курса робот рассматривался как материальная точка, способная мгновенно изменять вектор скорости. Однако реальный мобильный робот подчиняется **кинематическим связям и динамическим законам**: колесные платформы не могут смещаться вбок (неголономность), шины теряют сцепление при резких поворотах, а масса конструкции требует ненулевого тормозного пути.

Эта лекция посвящена **кинодинамическому планированию (планированию с учётом модели движения)**. Рассматриваются причины неприменимости чисто геометрических траекторий, методы модификации алгоритмов $A^*$ и RRT с учетом неголономности и инерции, а также переход к локальному предиктивному управлению (MPC) через алгоритмы DWA и MPPI.
...
2. **Иерархия моделей:** Кинематические (моноцикл, велосипед), кинодинамические и динамические. Сравнительный анализ.
3. **Hybrid $A^*$ и кривые Ридса–Шеппа:** 3D-дискретизация $(x, y, \theta)$, аналитическое решение BVP (кривые Дубинса и Ридса–Шеппа), расширение на цель.
...
10. **Model Predictive Path Integral (MPPI):** Стохастический ансамбль траекторий и Softmax-взвешивание без вычисления градиентов.
11. **Интерактивный симулятор:** Исследование алгоритма MPPI.
12. **Сравнительный анализ и выводы:** Сводная матрица методов.
...
- **Модель моноцикла / Unicycle (дифференциальный привод):**
  $$\dot{x} = v \cos\theta, \quad \dot{y} = v \sin\theta, \quad \dot{\theta} = \omega$$
  Управление: $u = (v, \omega)$. Разворот на месте ($R=0$).
- **Модель велосипеда / Bicycle (схема Аккермана):**
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Кинематическая (моноцикл / Unicycle)** | $(v, \omega)$ | 3D $(x, y, \theta)$ | Разворот на месте ($R=0$), минимальная вычислительная сложность | Пренебрежение инерцией, допущение бесконечных ускорений | Складские автономные платформы (AGV), сервисные роботы; маневрирование в тесных помещениях ($v < 1.5$ м/с) |
| **Кинематическая (велосипед / Bicycle)** | $(v, \delta)$ | 3D $(x, y, \theta)$ | Точное описание шасси с управляемой передней осью | $R_{\min} > 0$, невозможен разворот на месте | Беспилотные автомобили, колесные шасси автомобильного типа; движение и парковка без бокового скольжения |
| **Кинодинамическая** | $(a, \alpha)$ | 5D $(p, \theta, v, \omega)$ | Гладкость траектории класса $C^1$, непрерывность скоростей, гарантированный тормозной путь | Увеличение размерности задачи, пренебрежение проскальзыванием колес | Автономные колесные платформы средней скорости; движение с контролем перегрузок и тормозного пути |
...
<span>Пошаговое раскрытие вершин «▶ Шаг» | Непрерывный поиск «⚡ Запуск» | Аналитическое расширение (Ридса–Шеппа)</span>
...
<div class="card card-success text-sm mt-2">
...
<!-- _header: "Лекция 04 | Стохастическое предиктивное управление (MPPI)" -->

## 8. Стохастический метод MPPI (Model Predictive Path Integral) <span class="badge badge-time">70–76 мин</span>
...
### 1. Идея и ансамбль стохастических прогнозов (Rollouts)

Алгоритм DWA выбирает постоянную пару $(v, \omega)$ на коротком горизонте. При высоких скоростях и сложной динамике этого недостаточно.

**MPPI** оптимизирует **полную временную последовательность управлений** на горизонте $H$:
$$U = (u_0, u_1, \dots, u_{H-1})$$

1. Вокруг текущего плана $U$ генерируется ансамбль из $K$ случайных возмущений ($K = 10^3 \dots 10^4$):
   $$V^k = U + \delta U^k, \quad \delta u_t^k \sim \mathcal{N}(0, \Sigma)$$
2. Все $K$ вариантов **параллельно интегрируются вперед** по нелинейной модели динамики:
   $$x_{t+1} = f(x_t, v_t^k)$$
3. Для каждого прогноза вычисляется накопленный функционал стоимости $S(V^k)$ (штрафы за препятствия, отклонение от цели, усилия).
...
### 2. Экспоненциальное взвешивание (Softmax)

**Фундаментальное свойство:** отказ от вычисления градиентов ($\nabla_u S$) и линеаризации. Функция $S$ может быть разрывной и недифференцируемой (барьеры препятствий, срыв сцепления).

Оптимальное управление синтезируется как математическое ожидание по ансамблю:

<div class="formula-box text-xs">

$$w_k = \frac{\exp\left(-\frac{1}{\lambda} (S(V^k) - S_{\min})\right)}{\sum_{j=1}^K \exp\left(-\frac{1}{\lambda} (S(V^j) - S_{\min})\right)}, \quad u_t^* = u_t + \sum_{k=1}^K w_k \delta u_t^k$$

</div>

- **Температура $\lambda$:** регулирует селективность. При $\lambda \to 0$ выбирается наилучшая траектория ($\arg\min$), при $\lambda \to \infty$ — диффузное усреднение.
- **Штрафы за коллизии:** при риске аварии $S \gg 0 \implies w_k \to 0$.
- **Масштабируемость:** $10^4$ траекторий просчитываются на GPU в реальном времени с частотой 40–100 Гц (Nav2 MPPI, Isaac Gym).
...
<div class="card card-alert text-xs mt-2">

**Фундаментальное ограничение (дилемма «исследование vs. гладкость управления»):**
Ковариационная матрица шума $\Sigma$ одновременно определяет как **ширину охвата фазового пространства** (исследование альтернативных траекторий и выход из локальных минимумов), так и **высокочастотную зашумленность синтезируемого сигнала** (дребезг и рывки / *control chattering*). Высокая амплитуда белого шума расширяет область поиска, но перегружает приводы робота, что на практике требует применения фильтрации, цветного шума (процесс Орнштейна–Уленбека) или сплайновой параметризации.
...
## Разбор: параллельное прогнозирование траекторий и расчет весов в MPPI <span class="badge badge-time">76–80 мин</span>
...
## Интерактивный симулятор: Стохастический регулятор MPPI <span class="badge badge-time">80–85 мин</span>
...
5. Измените параметр «Дисперсия шума σ»: покажите студентам компромисс «исследование vs. дребезг» — большой шум шире охватывает пространство маневров, но приводит к высокочастотным изломам управляющего сигнала (chattering).
-->
...
- **Hybrid $A^*$** объединяет непрерывные кинематические траектории с пространственной фильтрацией состояний в 3D-сетке $(x, y, \theta)$, аналитическим расширением Ридса–Шеппа (Reeds–Shepp) и двойной эвристикой.
...
- **MPPI** реализует параллельное стохастическое прогнозирование тысяч нелинейных траекторий на GPU с вычислением оптимума через экспоненциальное взвешивание; требует балансировки компромисса «исследование vs. дребезг управления» (*chattering*).
...
- В чем заключается дилемма между шириной охвата пространства поиска и дребезгом управлений (*chattering*) в MPPI, и какими методами она разрешается?
