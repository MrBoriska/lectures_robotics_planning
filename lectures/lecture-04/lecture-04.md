...
**Движение только вперед ($v > 0$):**
По принципу максимума Понтрягина (ПМП) оптимальное управление для кинематики с ограниченным радиусом $R_{\min}$ является кусочно-постоянным ($u \in \{-1, 0, +1\}$).
- Траектория состоит только из дуг окружностей ($C \in \{L, R\}$) и прямолинейных отрезков ($S$).
- Существует ровно **6 канонических слов**:
  - Семейство $CSC$: $\{LSL, RSR, LSR, RSL\}$
  - Семейство $CCC$: $\{LRL, RLR\}$
- **Области применения:** БПЛА самолетного типа, ракеты, роботы без реверса.
...
**Движение вперед и назад ($v \in \{-1, +1\}$):**
Обобщение модели Дубинса на колесные системы с задним ходом (точки возврата / cusps со сменой знака скорости $|$).
- Кратчайший путь принадлежит одному из **48 канонических шаблонов** (9 базовых семейств):
  $$C|C|C, \quad CSC, \quad C|C_{\pi/2}C_{\pi/2}|C, \quad CC|C, \quad C|CC, \dots$$
- Обеспечивает маневры параллельной парковки и разворота в узком пространстве.
...
В общем случае двухточечная краевая задача (Two-Point BVP) для нелинейных систем требует итерационной численной оптимизации. Но для кинематики на плоскости кривые Дубинса и Ридса–Шеппа дают **точное замкнутое аналитическое решение за $< 1$ мкс**!
