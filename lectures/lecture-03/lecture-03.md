...
<span class="badge badge-purple">PRM • Lazy PRM • RRT • RRT-Connect • RRT* • Informed RRT* • BIT* • OMPL • cuRobo</span>
...
- **Синтезировать эвристики и сэмплинг:** понимать алгоритм **BIT\*** (Batch Informed Trees), объединяющий поиск $A^*$ на случайных графах с адаптивным сжатием в эллипс, и его применение в современных GPU-планировщиках (cuRobo).
...
## 5. Вычислительные структуры: $k$-d Tree и Continuous Collision Detection <span class="badge badge-time">69–72 мин</span>
...
## Вероятностная полнота vs Детерминированная полнота <span class="badge badge-time">72–75 мин</span>
...
<!-- _header: "Лекция 03 | Гибридное планирование" -->

## 6. Алгоритм BIT* (Batch Informed Trees): Синтез $A^*$ и сэмплинга <span class="badge badge-time">75–78 мин</span>
...
### Зачем объединять $A^*$ и Informed RRT*?

- **Проблема RRT\*:** Добавляет вершины поштучно, тратя ресурсы на переподключения (`Rewire`) в бесперспективных зонах.
- **Идея BIT\* (Gammell et al., 2015, 2020):** Объединить **детерминированный эвристический поиск $A^*$** (Лекция 02) с **пакетным сэмплированием** (Лекция 03).
- Планировщик работает не с отдельными точками, а с **пакетами (батчами) сэмплов**, строя случайный геометрический граф (RGG) возрастающей плотности.

</div>

<div class="card card-success mt-2">

### 4 столпа архитектуры BIT\*:

1. **Батчи сэмплов ($m$ точек):** Итеративное повышение разрешения без пересчёта всего пространства с нуля.
2. **Очередь рёбер по $A^*$:** Потенциальные рёбра $(u, v)$ упорядочиваются по эвристике допустимой стоимости:
   $$\hat{f}(u, v) = \hat{g}(u) + \|u - v\| + \hat{h}(v)$$
3. **Ленивая валидация (Lazy Evaluation):** Проверка на коллизии откладывается до момента извлечения лучшего ребра.
4. **Сжатие в эллипсоид:** После нахождения $c_{\text{best}}$ выборка запирается в эллипсоид $X_{\hat{f}}$, отсекая заведомо неоптимальные рёбра.
...
<img src="../../assets/images/lecture-03/bit_star_concept.svg" alt="BIT* Algorithm Concept" />
...
<!-- _header: "Интерактивная практика | Алгоритм BIT*" -->

## Интерактивный симулятор: Алгоритм BIT* <span class="badge badge-green">⏱️ 78–82 мин</span>

<div class="interactive-container">

<div class="interactive-header">

<span><i class="interactive-dot"></i> Алгоритм BIT*: пакетное сэмплирование, очередь рёбер A* и динамическое сжатие эллипса</span>
<span>Нажмите «📦 Батч» или «▶ Шаг A*» | Запустите «⚡ Запуск» | Наблюдайте сжатие эллипса и отложенную проверку коллизий</span>

</div>
<iframe src="http://localhost:5599/widgets/bit-star/index.html" class="interactive-frame"></iframe>

</div>

<!-- 
Методические указания лектору:
1. Нажмите «📦 Батч точек (+40)»: обратите внимание студентов на то, как генерируется облако сэмплов, а рёбра помещаются в очередь приоритетов A*.
2. Сделайте несколько шагов «▶ Шаг A*»: покажите, как исследуются наиболее перспективные рёбра с минимальным f = g + h.
3. Нажмите «⚡ Запуск»: после нахождения первого пути мгновенно появляется синий эллипс c_best, и все последующие батчи генерируются строго внутри него, сокращая длину пути до оптимума.
-->
...
## 7. Программный стек сэмплинга: OMPL и cuRobo <span class="badge badge-time">82–85 мин</span>
...
**OMPL** (*Open Motion Planning Library*) — эталонная C++ библиотека, лежащая в основе **MoveIt / MoveIt 2**:
...
- **Более 30 алгоритмов:** Реализации PRM, RRT, RRT*, Informed RRT*, KPIECE, FMT*, BIT* и др.
- **Архитектура:** Полная абстракция $\mathcal{C}$-пространства (`StateSpace`) от детекторов столкновений (`StateValidityChecker`).
- **Ограничение:** Однопоточные вычисления на CPU; планирование сложных движений занимает от сотен миллисекунд до нескольких секунд.
...
**cuRobo** (*CUDA Robotics*) — библиотека параллельного планирования движений от NVIDIA Research (ICRA 2023, Apache 2.0):

- **Массовый параллелизм на CUDA:** Вместо классического RRT на CPU использует параллельный сэмплинг (CuRobo-GP в духе BIT\*) и параллельную траекторную оптимизацию (L-BFGS / MPPI).
- **Прямая работа с GPU-картами:** Десятки миллионов проверок коллизий в секунду с полигональными сетками и воксельными ESDF (`nvblox`) в видеопамяти.
- **Быстродействие:** Полная генерация траектории 7-звенного робота занимает **20–50 миллисекунд** (в 60–80× быстрее OMPL + TrajOpt на CPU).
- **Реактивность:** Позволяет роботу на лету огибать динамические препятствия и людей с частотой перепланирования 20–50+ Гц.
...
## Сводная инженерная матрица сэмплирующих алгоритмов <span class="badge badge-time">85–88 мин</span>
...
| **BIT*** | Multi-Batch | Вероятностная | **Асимптотическая** | $\mathcal{O}(\|E\| \log \|V\|)$ | Синтез $A^*$ и сэмплинга, идеален для GPU | Чувствителен к размеру батча |
...
6. **BIT\*** объединяет эвристику $A^*$, сжатие в эллипсоид и батчевую обработку, формируя архитектурную основу современных GPU-планировщиков (**cuRobo**).
...
- За счет чего алгоритм BIT* сочетает преимущества $A^*$ и случайного сэмплирования?
