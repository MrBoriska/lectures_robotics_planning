...
### Решение: Informed RRT* (Gammell, 2014)

Выборка запирается в сжимающийся гиперэллипсоид:
$$\mathcal{C}_{\text{inf}} = \{ q \in \mathcal{C} \mid \|q - q_{\text{start}}\| + \|q - q_{\text{goal}}\| \le c_{\text{best}} \}$$
При улучшении пути ($c_{\text{best}} \downarrow$) эллипсоид сужается, фокусируя 100% вычислений вокруг глобального оптимума.
...
- **Проблема RRT\*:** Поштучная вставка сэмплов тратит время на локальный `Rewire` в бесперспективных зонах.
- **Идея BIT\* (Gammell, 2015):** Объединить **эвристический поиск $A^*$** (Лекция 02) с **пакетным сэмплированием** (Лекция 03).
- **Случайный граф (RGG):** Работа пакетами (батчами) сэмплов с итеративным уплотнением графа.
...
1. **Батчи ($m$ точек):** Итеративное повышение плотности сэмплов в пространстве.
2. **Очередь рёбер $A^*$:** упорядочивание кандидатов $(u, v)$ по $\hat{f}(u, v) = \hat{g}(u) + \|u - v\| + \hat{h}(v)$.
3. **Ленивая валидация:** Проверка коллизий запускается только при извлечении из очереди.
4. **Сжатие в эллипсоид:** После нахождения $c_{\text{best}}$ выборка запирается в эллипсоид $X_{\hat{f}}$.
