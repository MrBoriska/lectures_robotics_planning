...
Все интерактивные вставки в Markdown-исходниках лекций записаны каноническими относительными путями (`../../widgets/...`), поэтому сборка для GitHub Pages (`dist/`) работает нативно без строковых замен.

В VS Code предпросмотр работает через кастомный движок Marp (`scripts/marp-engine.cjs`), который на лету перенаправляет запросы к виджетам на локальный сервер (порт `5599`), исключая ошибки песочницы Chromium Webview. Сервер виджетов запускается **автоматически в фоновом режиме** при открытии проекта (через `postStartCommand` в DevContainer и фоновую задачу `.vscode/tasks.json`).
